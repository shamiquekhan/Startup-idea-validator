...
CRITICAL RULE: You may NOT introduce any new facts, numbers, competitors, or claims not present in the evidence. If the evidence is thin, say so rather than inventing.
...
4. Go-to-Market Suggestion — be specific about likely first-customer types (e.g., electric utilities, grid operators, industrial plants, SMBs, enterprise procurement teams, etc.)
5. Key Risks
...
def _fallback_plan(report: ValidationReport) -> str:
    ...
    target = report.intake.target_user if report.intake.target_user != "not specified" else "potential early adopters"
    return (
        ...
        f"Likely first customers for this type of solution include: "
        f"{target} directly, or adjacent segments that share the same problem. "
        f"Consider starting with a niche vertical to prove traction before expanding.\n\n"
        f"## Key Risks\n{risks}\n\n"
        f"## Next Validation Steps\n"
        f"1. Interview {target} to validate the problem\n"
